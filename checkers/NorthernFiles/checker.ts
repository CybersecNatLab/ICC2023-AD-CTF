...
import * as checklib from './checklib';
...
const checkerFetch: typeof fetch = async (input: RequestInfo | URL, init?: RequestInit | undefined) => {
...
async function check_sla() {
	try {
		...
		assert(
			page.indexOf('<ListBucketResult xmlns="http://s3.amazonaws.com/doc/2006-03-01/">') !== -1,
				...
				try {
					if (randomChoice([true, false])) {
						...
						return await api.getFile(
							...
							password2,
							undefined,
							await api.getPrivateKey(username2, password2)
						);
					}
				} catch (e) {
...
(async () => {
	try {
		if (action == checklib.Action.CHECK_SLA) await check_sla();
		else if (action == checklib.Action.PUT_FLAG) await put_flag();
		else if (action == checklib.Action.GET_FLAG) await get_flag();
	} catch (e) {
